...
export function ReportsView({ month, unitName, activeUnitId, totals, scales, targets }: { month: string; unitName: string; activeUnitId: string | null; totals: Total[]; scales: ScaleRow[]; targets: IndicatorTarget[] }) {
  ...
  const rows = totals.filter((t) => t.total != null).map((t) => {
    const total = Number(t.total);
    const target = effectiveTarget(targets, t.indicator_id, activeUnitId, null);
    return { ...t, total, situation: targetSituation(total, target) };
  ...
  function pdf() {
    const doc = new jsPDF(); doc.setFontSize(18); doc.text("MaisFisio — Relatório mensal", 16, 20); doc.setFontSize(11); doc.text(`Competência: ${month.split("-").reverse().join("/")} · ${unitName}`, 16, 29);
    ...
    rows.forEach((r) => { doc.text(r.indicator_name.slice(0, 78), 16, y); doc.text(r.kind === "taxa" ? `${ptBRNumber.format(r.total)}%` : ptBRNumber.format(r.total), 190, y, { align: "right" }); y += 5; if (r.situation) { const targetValue = `${ptBRNumber.format(Number(r.situation.target.target_value))}${r.kind === "taxa" ? "%" : ""}`; doc.text(`${r.situation.achieved ? "Meta atingida" : "Meta não atingida"} · alvo ${r.situation.target.comparison === "maximo" ? "máximo" : "mínimo"} ${targetValue}`, 20, y); y += 5; } else { y += 1; } if (y > 275) { doc.addPage(); y = 20; } });
