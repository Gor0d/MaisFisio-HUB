...
type AuditRow = { id: number; changed_at: string; action: string; table_name: string; record_id: string | null; changed_by_name: string | null };
...
function Collaborators({ role, currentUserId, currentServiceId, units, activeUnitId, services, profiles, profileUnits, collaborators, collaboratorUnits }: { role: string; currentUserId: string; currentServiceId: string | null; units: UnitRow[]; activeUnitId: string | null; services: ServiceRow[]; profiles: ProfileRow[]; profileUnits: ProfileUnitRow[]; collaborators: CollaboratorRow[]; collaboratorUnits: CollaboratorUnitRow[] }) {
  ...
  async function invite(formData: FormData) {
    ...
    toast.success(data.message ?? "Convite enviado e acesso configurado.");
    location.reload();
  }
...
function Indicators({ indicators, canManage }: { indicators: IndicatorRow[]; canManage: boolean }) { async function toggle(id: string, active: boolean) { const { error } = await createClient().from("indicators").update({ active: !active }).eq("id", id); if (error) toast.error(error.message); else location.reload(); } return <Card><CardHeader><CardTitle>Catálogo de indicadores</CardTitle><CardDescription>{canManage ? "Indicadores são dados: ativar ou desativar não exige alterar o banco." : "Catálogo global em modo de consulta. Somente a matriz pode alterá-lo."}</CardDescription></CardHeader><CardContent className="divide-y p-0">{indicators.map((x) => <div key={x.id} className="flex items-center gap-4 px-5 py-3 md:px-6"><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{x.name}</p><p className="text-xs text-muted-foreground">{x.services?.[0]?.name} · {x.context} · {x.kind}</p></div>{canManage ? <Button size="sm" variant={x.active ? "outline" : "secondary"} onClick={() => toggle(x.id, x.active)}>{x.active ? "Desativar" : "Ativar"}</Button> : <Badge>{x.active ? "Ativo" : "Inativo"}</Badge>}</div>)}</CardContent></Card>; }

function Audit({ rows }: { rows: AuditRow[] }) { const action: Record<string, string> = { INSERT: "Criação", UPDATE: "Alteração", DELETE: "Exclusão" }; return <Card><CardHeader><CardTitle>Trilha de auditoria</CardTitle><CardDescription>Últimas 100 alterações sensíveis da unidade ativa.</CardDescription></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-y bg-muted/50 text-xs uppercase text-muted-foreground"><tr><th className="px-6 py-3">Data</th><th className="px-6 py-3">Ação</th><th className="px-6 py-3">Entidade</th><th className="px-6 py-3">Responsável</th><th className="px-6 py-3">Registro</th></tr></thead><tbody className="divide-y">{rows.map((x) => <tr key={x.id}><td className="whitespace-nowrap px-6 py-3">{new Date(x.changed_at).toLocaleString("pt-BR")}</td><td className="px-6 py-3"><Badge>{action[x.action]}</Badge></td><td className="px-6 py-3">{x.table_name}</td><td className="px-6 py-3">{x.changed_by_name ?? "Sistema"}</td><td className="max-w-40 truncate px-6 py-3 font-mono text-xs">{x.record_id}</td></tr>)}</tbody></table></CardContent></Card>; }
