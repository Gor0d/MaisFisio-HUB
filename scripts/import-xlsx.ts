...
async function upload(production: ProductionImport[], scales: ScaleImport[]) {
  ...
  const [services, sectors, indicators, items, options, existingCollaborators] = await Promise.all([allRows(client,"services","id,code"), allRows(client,"sectors","id,code"), allRows(client,"indicators","id,code"), allRows(client,"scale_items","id,scale_type,code"), allRows(client,"scale_item_options","id,item_id,points"), allRows(client,"collaborators","id,normalized_name,service_id")]);
  ...
  const patientRows = [...new Map(scales.map((x) => [x.recordNumber, x])).values()].map((x) => ({ initials: x.initials, record_number: x.recordNumber, age: x.age })); const patientMap = new Map<string,string>();
  await batches(patientRows, 500, async (batch) => { const { data, error } = await client.from("patients").upsert(batch, { onConflict: "record_number" }).select("id,record_number"); if (error) throw error; data?.forEach((x) => patientMap.set(x.record_number, x.id)); });
  await batches(production, 200, async (batch) => {
    const records = batch.map((x) => ({ id: stableUuid(`production:${x.service}:${x.date}:${x.shift}:${x.sector}:${normalizeKey(x.collaborator)}:${x.context}`), service_id: serviceMap.get(x.service), record_date: x.date, shift: x.shift, sector_id: sectorMap.get(x.sector), sector_type: x.sectorType, collaborator_id: collaboratorMap.get(`${serviceMap.get(x.service)}:${normalizeKey(x.collaborator)}`), context: x.context, created_by: actor }));
  ...
  await batches(scales, 200, async (batch) => {
    const assessments = batch.map((x) => ({ id: stableUuid(`scale:${x.source}:${x.row}:${x.recordNumber}`), scale_type: x.scaleType, patient_id: patientMap.get(x.recordNumber), collaborator_id: x.collaborator ? collaboratorMap.get(`${serviceMap.get("fisioterapia")}:${normalizeKey(x.collaborator)}`) : null, assessment_date: x.date, moment: x.moment, sector_id: sectorMap.get(x.sector), sector_type: x.sectorType, attendance_number: x.attendanceNumber, cid: x.cid, event_date: x.eventDate, notes: x.notes, created_by: actor }));
