...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  let metricsQuery = supabase.from("production_metrics").select("record_date,indicator_code,indicator_name,kind,value,service_id,sector_id,unit_id").gte("record_date", start).lte("record_date", end).order("record_date");
  if (activeUnitId) metricsQuery = metricsQuery.eq("unit_id", activeUnitId);
