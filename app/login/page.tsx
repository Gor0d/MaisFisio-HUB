...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ redirect?: string; erro?: string }> }) {
  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-[1.05fr_.95fr]">
      <section className="relative hidden overflow-hidden bg-[#16233a] p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative max-w-xl">
          ...
          <p className="mt-6 max-w-lg text-lg leading-relaxed text-blue-50/80">Indicadores assistenciais confiáveis para transformar o trabalho diário em decisões melhores.</p>
          <div className="mt-10 grid grid-cols-3 gap-4">
            {[[Activity, "Escalas calculadas"], [BarChart3, "Indicadores em tempo real"], [ShieldCheck, "Dados protegidos"]].map(([Icon, text]) => {
              ...
              return <div key={String(text)} className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur"><LucideIcon className="mb-3 size-5 text-blue-200" /><p className="text-sm text-blue-50/80">{String(text)}</p></div>;
            })}
          </div>
        </div>
      ...
      <section className="flex items-center justify-center p-5 sm:p-10">
        <div className="w-full max-w-md">
          <div className="mb-8 flex justify-center lg:hidden">
            <Image src="/logo-maisfisio.png" alt="MaisFisio" width={240} height={160} priority style={{ height: "auto", width: "auto", maxHeight: 72 }} />
          </div>
